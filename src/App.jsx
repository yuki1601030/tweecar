...
function SceneImageViewer({ scene }) {
  return <div className="image-viewer">
    <img src={scene.image} alt={`${scene.situation} シーン${scene.sceneNo} の状況画像`} onError={(e) => e.currentTarget.parentElement.classList.add('missing')} />
...
function Vehicle({ id, lane, bottom, moved }) { if (bottom == null) return null; return <div className={`vehicle ${id.toLowerCase()} ${moved ? 'moved' : ''}`} style={{ left: `calc(${lanePos[lane]}% - 38px)`, bottom: `${bottom}%` }}><span>{id}</span></div>; }
...
function NavPanel({ scene, acted }) {
  ...
  return <section className="nav-panel">
    <div className="panel-head"><span>DRIVER NAV</span><b>高速道路走行中</b><em className={`risk ${nav.risk.toLowerCase()}`}>{nav.risk}</em></div>
...
function ConversationPanel({ scene, speaking }) { return <section className="conversation-panel"><div className="panel-head"><span>VOICE CONVERSATION</span><b>{speaking ? 'AIが発話中...' : '待機中'}</b></div><div className="bubble ai"><span className="ico">🤖</span><div>{scene.ai.map((line, i) => <p key={i}>{line}</p>)}<Wave active={speaking}/></div><span className="speaker">🔊</span></div><div className="bubble driver"><span className="ico">👤</span><div><p>{scene.driver}</p></div></div>{scene.resultCards?.length && <div className="result-cards">{scene.resultCards.map(card => <span key={card}>{card}</span>)}</div>}</section>; }

function InternalData({ scene, behind, setBehind }) { const rows = behind ? scene.internal : scene.packets; return <aside className="internal-data"><div className="panel-head"><span>Behind the scenes</span><button onClick={() => setBehind(!behind)} className={behind ? 'on' : ''}>{behind ? <span>👁️</span> : <span>🙈</span>}内部データを見る</button></div><div className="data-list">{rows.map((row, i) => <p key={i} className={behind ? 'raw' : ''}>{row}</p>)}</div></aside>; }
...
function DriveConsole({ scene, current, setScene }) {
  ...
  const actions = useMemo(() => scene.actions, [scene]);
...
export default function App() {
  const [current, setCurrent] = useState(0); const [intro, setIntro] = useState(true); const scene = scenes[current];
  return <main className="app"><SceneHeader scene={scene} current={current} setScene={setCurrent}/><DriveConsole scene={scene} current={current} setScene={setCurrent}/>{intro && <div className="intro"><div><h2>TweeCar</h2><p>クルマたちは裏側で情報を共有し、<br/>あなたには必要な情報だけ届きます。</p><button onClick={() => setIntro(false)}>体験を始める</button></div></div>}</main>;
}
